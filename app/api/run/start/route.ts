...
function resolveProjectPath(request: Request, payload: StartRunPayload): string | null {
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (parsed !== null && typeof parsed === "object" && !Array.isArray(parsed)) {
      payload = parsed as StartRunPayload;
    } else {
  ...
  if (payload.prdPath !== undefined && typeof payload.prdPath !== "string") {
    return NextResponse.json({ error: "prdPath must be a string." }, { status: 400 });
  }
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Unexpected process start error.";
